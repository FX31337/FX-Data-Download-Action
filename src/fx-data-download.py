...
class Dukascopy:
        ...
        if os.path.isfile(self.path):
            ...
            while i <= 5:
                try:
                    ...
                    print(
                        "Error: The downloaded data is less than the expected amount, so skipping."
                    )
                    i += 1
    ...
    def bt5_to_csv(self):
        ...
        # Workaround for liblzma bug (https://bugs.python.org/issue21872)
            ...
            print(
                "Info: Ran into liblzma decompressor bug, falling back to command line decompression..."
            )
            try:
                pipe = subprocess.Popen(
                    ["xz", "-dc", self.path], stdout=subprocess.PIPE
                )
            except FileNotFoundError:
                print(
                    "Error: Unable to find the 'xz' LZMA decompressor utility in your PATH, moving on."
                )
                return False
